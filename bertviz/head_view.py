...
def output_head_view_html(attention, tokens, sentence_b_start = None, prettify_tokens=True):
    """output head view html file
    ...
    __location__ = os.path.realpath(
        os.path.join(os.getcwd(), os.path.dirname(__file__)))
    vis_js = open(os.path.join(__location__, 'head_view.js')).read()
    vis_standalone_js = open(os.path.join(__location__, 'head_view_standalone.js')).read()
    ...
    requirejs_src = '<script src="https://cdnjs.cloudflare.com/ajax/libs/require.js/2.3.6/require.min.js"></script>'
    data_script = '<script>\n'+'window.params = %s' % json.dumps(params)+'\n</script>'
    vis_script = f"<script>\n{vis_standalone_js}\n</script>"

    standalone_vis_html = requirejs_src + data_script + vis_script + vis_html

    return standalone_vis_html
